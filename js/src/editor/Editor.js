...
Editor.prototype.draw = function ()
{
	...
	if(this._shape.available)
	{
		this._shape.erase();
		this._shape.reset();
	}

	if(this.slice && this._anchorsVisible)
	{
		this._anchorsVisible = false;
		$("#js-style").html("#editor .anchor{display: none;}");
	}

	if(!this.slice && !this._anchorsVisible)
	{
		this._anchorsVisible = true;
		$("#js-style").html("#editor .anchor{display: block;}");
	}

	// slicer
	this._slicer.locked = !this.slice;
